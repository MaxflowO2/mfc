...
import (
	...
	"github.com/MaxflowO2/mfc/cryptosign/dilithium/internal/common/params"
)
...
func makeHintAVX2() {
	...
	gamma2PlusOneX8 := YMM() // γ_2 + 1
	minGamma2X8 := YMM()     // q - γ_2
	zeroX8 := YMM()
	signMaskX8 := YMM()

	ret := GP32()
	XORL(ret, ret) // zero ret

	VPXOR(zeroX8, zeroX8, zeroX8)
	broadcastImm32(params.Gamma2+1, gamma2PlusOneX8)
	broadcastImm32(params.Q-params.Gamma2, minGamma2X8)
	broadcastImm32(0x80000000, signMaskX8)

	var x0 [4]VecVirtual
	var t [4]VecVirtual
	var h [4]VecVirtual
	signBits := []Op{GP32(), GP32(), GP32(), GP32()}
	for i := 0; i < 4; i++ {
		x0[i] = YMM()
		t[i] = YMM()
		h[i] = YMM()
	...
	for j := 0; j < 8; j++ {
		for i := 0; i < 4; i++ {
			VMOVDQU(Mem{Base: p0Ptr, Disp: 32 * (4*j + i)}, x0[i])
		}

		// The hint is 0 if x₀ ⩽ γ_2 or x₀ > q - γ_2 or finally
		// if x₀ = q - γ_2 and x₁ = 0.  Otherwise the  hint is 1.
		// See makeHint() for an explanation.

		// x₀ ⩽ γ_2 is equivalent to x₀ - (γ_2 + 1) < 0.
		// We store this in the sign bit of t.
		for i := 0; i < 4; i++ {
			VPSUBD(gamma2PlusOneX8, x0[i], h[i])
		}

		// q - γ_2 < x₀ iff (q - γ_2) - x₀ < 0.
		for i := 0; i < 4; i++ {
			VPSUBD(x0[i], minGamma2X8, t[i])
		}
		for i := 0; i < 4; i++ {
			VPOR(h[i], t[i], h[i])
		}

		// Check x₀ = q - γ_2 and x₁ = 0
		for i := 0; i < 4; i++ {
			VPCMPEQD(minGamma2X8, x0[i], t[i])
		}
		// x0 is no long used, so we'll use it to store whether x₁ = 0.
		for i := 0; i < 4; i++ {
			VPCMPEQD(Mem{Base: p1Ptr, Disp: 32 * (4*j + i)}, zeroX8, x0[i])
		}
		for i := 0; i < 4; i++ {
			VPAND(x0[i], t[i], t[i])
		}
		...
		// The signbit of h is 1 iff the hint should be 0, so invert
		for i := 0; i < 4; i++ {
			VPXOR(h[i], signMaskX8, h[i])
		}
		for i := 0; i < 4; i++ {
			VPAND(h[i], signMaskX8, h[i])
		}
		for i := 0; i < 4; i++ {
			VPMOVMSKB(h[i], signBits[i])
		}
		for i := 0; i < 4; i++ {
			VPSRLD(U8(31), h[i], h[i])
		}
		for i := 0; i < 4; i++ {
			VMOVDQU(h[i], Mem{Base: pPtr, Disp: 32 * (4*j + i)})
		}

		for i := 0; i < 4; i++ {
			tmp := GP32()
			POPCNTL(signBits[i], tmp)
			ADDL(tmp, ret)
		}
	}
